...
expected_keys = ['NIK', 'Nama','Tempat/Tgl Lahir','Jenis Kelamin', 'Alamat', 'RT/RW','Kel/Desa', 'Kecamatan','Agama', 'Status Perkawinan','Pekerjaan','Kewarganegaraan','Berlaku Hingga'] 
...
# Fungsi untuk ekstraksi NIK pada KTP
def extract_nik(img):
    target_field = "NIK"
    reader =easyocr.Reader(['id'])
    results = reader.readtext(img)
    for i, (_, text, _) in enumerate(results):
        if target_field.lower() in text.lower():
            # Get the next item to NIK
            if i + 1 < len(results):
                return results[i + 1][1].strip()

# Fungsi untuk validasi NIK pada KTP berdasarkan jumlah digit
def validate_nik(img):
    nik_number = extract_nik(img)
    return len(nik_number) == 16
